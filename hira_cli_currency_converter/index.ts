...
let apiLink =
  "https://v6.exchangerate-api.com/v6/20a83004552edd35702b2986/latest/PKR";
...
let userAmount = await inquirer.prompt([
  {
    type: "number",
    name: "rupees",
    message: `Please enter your amount in ${chalk.greenBright.bold(
      selectFirstCountry.firstCountryName
    )} ?`,
  },
]);
...
let dataConversionRate = async (data: any) => {
...
console.log(
  `Your ${chalk.yellowBright.bold(
    selectFirstCountry.firstCountryName
  )} ${chalk.yellowBright.bold(userAmount.rupees)} ${chalk.yellowBright.bold(
    selectSecondCountry.secondCountryName
  )} ${chalk.yellowBright.bold(convertedRate)}`
);
