...
for (let i = 1; i <= 5; i++) {
    ...
    const customers = new Customer(fName, lName, 22 * i, "female", contactNum, 1000 + i);
...
async function bankService(bank) {
    do {
        ...
        if (service.select == "View Balance") {
            ...
            //find account number
            let account = UnitedBank.account.find((acc) => acc.accountNum == response.accountNum);
            //use if conditional statement
            if (!account) {
                console.log(chalk.red.bold("Invalid Account Number ⚠️"));
            }
            if (account) {
                let name = UnitedBank.customer.find((item) => item.accountNumber == account?.accountNum);
                console.log(`Dear "${chalk.blue.italic.bold(name?.firstName)} ${chalk.cyan.italic(name?.lastName)} your Account Balance is ${chalk.bold.bgMagenta(`💲${account.bankBalance}`)}`);
            }
        }
        // Cash Withdraw
